...
const benefitCard: React.FC<Props> = ({ title, description, image, className }) => {
    return (
        <div
            className={`group flex flex-col items-center bg-gradient-to-b py-10 px-6 shadow ${className ? ` ${className}` : ""
                }`}
        >
            {image && (
